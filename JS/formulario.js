  ...
  if (esEdicion && registroEditar) {
    Object.keys(registroEditar).forEach((key) => {
      ...
      if (input.type === "checkbox") {
        const valor = String(registroEditar[key]).toLowerCase();
        ...
        input.value = convertirFechaInput(registroEditar[key]);
      } else {
...
form.addEventListener("submit", async function (e) {
  ...
  columnas.forEach((campo) => {
    if (campo.nombre.toLowerCase() !== "id") {
      ...
      if (input.type === "checkbox") {
        registro[campo.nombre] = input.checked ? "si" : "no";
      } else {
        registro[campo.nombre] = input.value;
      }
    }
  });
...
function crearInputPorTipo(tipo, campo = null) {
  let elemento;
...
function crearCampoFormulario(campo) {
  ...
  const elemento = crearInputPorTipo(tipo, campo);
  elemento.id = nombre;
